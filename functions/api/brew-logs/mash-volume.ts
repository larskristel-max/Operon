...
interface Body { batchId?: string; actualMashVolumeLiters?: number }
...
export async function onRequestPost(context: { request: Request; env: BreweryEnv }): Promise<Response> {
  ...
  const liters = Number(body.actualMashVolumeLiters);
  if (!body.batchId || !Number.isFinite(liters) || liters <= 0) return jsonResponse({ error: "Valid batchId and actualMashVolumeLiters > 0 are required" }, 400);
  ...
  if (existingId) {
    const patchRes = await fetch(
      ...
      { method: "PATCH", headers: adminHeaders(context.env), body: JSON.stringify({ actual_mash_volume_liters: liters, log_status: "in_progress", updated_at: now }) }
    );
    const patched = await patchRes.json();
    if (!patchRes.ok) return jsonResponse({ error: "Failed to update mash volume", detail: patched }, 400);
  ...
  const insertRes = await fetch(`${context.env.SUPABASE_URL}/rest/v1/brew_logs`, {
    method: "POST",
    headers: adminHeaders(context.env),
    body: JSON.stringify({ brewery_id: breweryId, batch_id: body.batchId, actual_mash_volume_liters: liters, log_status: "in_progress", created_at: now, updated_at: now }),
  });
  const inserted = await insertRes.json();
  if (!insertRes.ok) return jsonResponse({ error: "Failed to record mash volume", detail: inserted }, 400);
