...
begin
  ...
  -- SELECT: same-brewery visibility (read-only across tenant)
  drop policy if exists users_self_or_same_brewery on public.users;
  drop policy if exists users_select_same_brewery on public.users;
  create policy users_select_same_brewery
    on public.users
    for select
    to authenticated
    using (
  ...
  -- UPDATE: self-only AND can't change own brewery_id
  ...
  create policy users_update_self
    ...
    with check (
      id = auth.uid()
      and brewery_id = (auth.jwt() ->> 'brewery_id')::uuid
    );

  -- No INSERT or DELETE policies for authenticated role:
  --   user provisioning + deletion happen via Cloudflare Functions with service role,
  --   which bypass RLS as intended. Phase 2.F (RBAC) will introduce admin-role write paths.
